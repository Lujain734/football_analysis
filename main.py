...
def parse_args():
    ...
    parser.add_argument(
        "--video-path",
        type=str,
        default=r"input_videos/header.mp4",
    ...
    parser.add_argument(
        ...
        type=float,
        nargs=2,
        help="x y of target player in early frames (e.g., 708 446)",
    )
    parser.add_argument(
        "--crop-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.3,
        help="Zoom factor for target crop (e.g., 1.3 or 1.4)",
    )
    parser.add_argument(
        "--crop-size",
        type=int,
        default=224,
        help="Output crop size (square). e.g., 224 or 256",
    )
    parser.add_argument(
        "--iou-thr",
        type=float,
    ...
    parser.add_argument(
        "--pose-weights",
        type=str,
        default=r"models/best1.pt",
        help="Path to YOLOv8 Pose weights",
    )
    parser.add_argument(
        "--mlp-weights",
        type=str,
        default=r"models/action_mlp.pt",
...
def main():
    ...
    # Seed target from early frames
    target_id = None
    last_bbox = None
    if args.target_xy:
        f0, pid0, bbox0 = seed_target_from_first_k_frames(tracks["players"], args.target_xy, K=10)
        if pid0 is not None:
            target_id = pid0
            last_bbox = bbox0
    ...
    for f_idx, frame in enumerate(video_frames):
        ...
        if target_bbox:
            ...
            # highlight target in output video (visual only)
