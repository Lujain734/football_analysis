...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        nargs=2,
        help="x y of target player in early frames (normalized 0-1)",
    )
    parser.add_argument(
        "--crop-dir",
        type=str,
...
def seed_target_from_first_k_frames(tracks_players, target_xy, K=10):
    ...
    for f_idx in range(max_f):
        for pid, tr in tracks_players[f_idx].items():
            ...
            # 🔥 طباعة تشخيصية
            if f_idx == 0:  # في الفريم الأول فقط
...
def main():
    ...
    # 🔥 تحويل الإحداثيات من normalized (0-1) إلى pixel coordinates
    target_xy = None
    if args.target_xy:
    ...
    target_id = None
    last_bbox = None
    if target_xy:  # 🔥 استخدم target_xy المحولة
