...
def parse_args():
    parser = argparse.ArgumentParser(description="Football Analysis with Target Player Tracking + Cropping + Action Counting")
    parser.add_argument("--video-path", type=str, default=r"input_videos/header.mp4", help="Path to input video")
    parser.add_argument("--target-xy", type=float, nargs=2, help="x y of target player in early frames (normalized 0-1)")
    parser.add_argument("--crop-dir", type=str, default="target_crops", help="Directory to save cropped target images")
    parser.add_argument("--zoom", type=float, default=1.3, help="Zoom factor for target crop (e.g., 1.3 or 1.4)")
    parser.add_argument("--crop-size", type=int, default=224, help="Output crop size (square). e.g., 224 or 256")
    parser.add_argument("--iou-thr", type=float, default=0.03, help="IoU threshold for ID fallback continuity")
    parser.add_argument("--pose-weights", type=str, default=r"models/best1.pt", help="Path to YOLOv8 Pose weights")
    parser.add_argument("--mlp-weights", type=str, default=r"models/action_mlp.pt", help="Path to 34D MLP weights")
    parser.add_argument("--conf-thr", type=float, default=0.75, help="Classification confidence threshold for accepting an action")
    parser.add_argument("--yolo-conf", type=float, default=0.25, help="YOLO pose confidence threshold")
    parser.add_argument("--img-size", type=int, default=736, help="YOLO pose inference image size")
    parser.add_argument("--max-det", type=int, default=5, help="Max detections per image for pose")
    parser.add_argument("--smooth-window", type=int, default=7, help="Smoothing window length over probabilities")
    parser.add_argument("--min-seg-sec", type=float, default=0.30, help="Minimum duration (sec) for an action segment to be counted")
...
def seed_target_from_first_k_frames(tracks_players, target_xy, K=10):
    ...
    print(f"\n🔍 Looking for player near: ({target_xy[0]:.1f}, {target_xy[1]:.1f})")
    ...
    for f_idx in range(max_f):
        for pid, tr in tracks_players[f_idx].items():
            ...
            if f_idx == 0:
                print(f"   Player {pid}: center=({cx:.1f}, {cy:.1f}), distance={d:.1f}")
    ...
    if best[1] is not None:
        print(f"✅ Best match: Player {best[1]}, distance={best_dist:.1f}\n")
    else:
        print(f"❌ No player found near target coordinates\n")
...
def main():
    ...
    if args.target_xy:
        ...
        pixel_x = norm_x * W
        pixel_y = norm_y * H
        target_xy = (pixel_x, pixel_y)
        print(f"🎯 Normalized coords: ({norm_x:.3f}, {norm_y:.3f})")
        print(f"🎯 Pixel coords: ({pixel_x:.1f}, {pixel_y:.1f})")
    ...
    if target_xy:
        ...
        if pid0 is not None:
            ...
            print("⚠️ No player near the provided coordinates in first frames.")
            print(f"   Tried position: ({target_xy[0]:.1f}, {target_xy[1]:.1f})")
    ...
    output_video_frames = tracker.draw_annotations(video_frames, tracks, team_ball_control, target_id=target_id)
    ...
    for f_idx, frame in enumerate(video_frames):
        ...
        if target_bbox:
            ...
            cv2.putText(output_video_frames[f_idx], "TARGET", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
